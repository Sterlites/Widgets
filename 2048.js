...
(function() {
        ...
        try {
                ...
                for (let col = 0; col < 3; col++) {
                    if (grid[row][col] > 0 && grid[row][col+1] > 0) {
                        ...
                        if (currentPattern === 'snake') {
                            ...
                            if ((colDirection === 1 && grid[row][col] >= grid[row][col+1]) ||
            ...
            for (let col = 0; col < 4; col++) {
                for (let row = 0; row < 3; row++) {
                    if (grid[row][col] > 0 && grid[row+1][col] > 0) {// Adapt based on the target corner
                        const rowDirection = targetCorner.row === 0 ? 1 : -1;
                        if ((rowDirection === 1 && grid[row][col] >= grid[row+1][col]) ||
            ...
            if (maxPos.row === targetCorner.row && maxPos.col === targetCorner.col) {
                ...
                // Check row chain
                for (let c = targetCorner.col + colDir, steps = 1; c >= 0 && c < 4 && steps < 4; c += colDir, steps++) {
                ...
                // Check column chain
                for (let r = targetCorner.row + rowDir, steps = 1; r >= 0 && r < 4 && steps < 4; r += rowDir, steps++) {
                ...
                const distance = Math.abs(maxPos.row - targetCorner.row) + Math.abs(maxPos.col - targetCorner.col);
            ...
            for (let row = 1; row < 3; row++) {
                for (let col = 1; col < 3; col++) {
                    if (grid[row][col] > 0) {
                        ...
                        const avgNeighbor = neighbors.reduce((sum, val) => sum + val, 0) / Math.max(1, neighbors.length);
    ...
    function lookAheadEvaluation(grid, depth, alpha, beta, maximizingPlayer, originalDepth) {
        ...
        if (maximizingPlayer) {
            ...
            for (const direction of directions) {
                ...
                if (moved) {
                    const result = lookAheadEvaluation(nextGrid, depth - 1, alpha, beta, false, originalDepth);
            ...
            for (const cell of emptyCells) {
                ...
                if (strategic) {
                    ...
                    const index = randomCells.findIndex(c => c.row === cell.row && c.col === cell.col);
            ...
            const sampledCells = [];
            while (sampledCells.length < sampleSize && (strategicCells.length > 0 || randomCells.length > 0)) {
            ...
            for (const cell of sampledCells) {
                ...
                gridWith2[row][col] = 2;
                const resultWith2 = lookAheadEvaluation(gridWith2, depth - 1, alpha, beta, true, originalDepth);
                
                // Try with 4 (10% probability)
                const gridWith4 = JSON.parse(JSON.stringify(grid));
                gridWith4[row][col] = 4;
                const resultWith4 = lookAheadEvaluation(gridWith4, depth - 1, alpha, beta, true, originalDepth);
                ...
                if (combinedScore < worstScore) {
                    ...
                    worstMoves = resultWith2.score <= resultWith4.score ? 
                        resultWith2.moves : resultWith4.moves;
                }
...
function decideNextMove() {
    ...
    if (moveCount % 20 === 0 || moveCount === 0) {
        ...
        if (DEBUG) console.log(`Updated target corner to: row ${targetCorner.row}, col ${targetCorner.col}`);
    ...
    if (potentialDeadlock) {
        ...
        if (DEBUG) console.log("DEADLOCK RISK DETECTED! Increasing search depth.");
    ...
    if (validMoves.length > 1) {
        ...
        const relativeDiff = scoreDiff / Math.max(1, Math.abs(validMoves[0].score));
        ...
        if (potentialDeadlock && validMoves.length >= 3) {
            ...
            const thirdRelativeDiff = thirdScoreDiff / Math.max(1, Math.abs(validMoves[0].score));
        ...
        if (Math.random() < 0.02 && moveCount > 50) {
            const randomIndex = Math.floor(Math.random() * Math.min(3, validMoves.length));
    ...
    if (DEBUG && moveCount % 20 === 0) {
        console.log("Planned moves:", validMoves[0].moves.slice(0, 3).join(" → "));
    }
    ...
    try {
        ...
        if (nextMove) {
            ...
            if (moveCount % 100 === 0) {
                ...
                console.log(`Target corner: Row ${targetCorner.row}, Col ${targetCorner.col}`);
            }

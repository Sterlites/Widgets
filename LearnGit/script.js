document.addEventListener("DOMContentLoaded", () => {
  ...
  tabButtons.forEach((button) => {
    button.addEventListener("click", () => {
      // Remove active class from all tabs and command groups
      tabButtons.forEach((btn) => btn.classList.remove("active"));
      commandGroups.forEach((group) => group.classList.remove("active"));

      // Add active class to clicked tab and corresponding command group
  ...
  const btnPushFeatureUpstream = document.getElementById(
    "btn-push-feature-upstream"
  );
  ...
  const remoteRepoContent = document.querySelector(
    "#remote-repo .repo-content"
  );
  ...
  const COMMIT_NODE_SIZE = 20; // Adjusted for compact layout
  const VERTICAL_SPACING = 40; // Reduced vertical spacing
  const HORIZONTAL_SPACING = 40; // Reduced horizontal spacing
  ...
  function renderFilesUI() {
    ...
    // Render repository files in commit nodes
    renderRepositoryFiles();
  }

  // Get DOM elements for repository files
  const localRepoFiles = document.getElementById("local-repo-files");
  const remoteRepoFiles = document.getElementById("remote-repo-files");

  // Function to show files in the repository (both in commit nodes and in the repository file areas)
  function renderRepositoryFiles() {
    // Clear previous repository files
    localRepoFiles.innerHTML = "";
    remoteRepoFiles.innerHTML = "";

    // --- Local Repository Files ---
    // Get the current HEAD commit
    const headCommitHash = localRepo.head
      ? localRepo.head.type === "branch"
        ? localRepo.branches[localRepo.head.target]
        : localRepo.head.target
      : null;

    if (headCommitHash && localRepo.commits[headCommitHash]) {
      const headCommit = localRepo.commits[headCommitHash];

      // Find the commit node in the visualization
      const commitNode = localRepoContent.querySelector(
        `.commit-node[data-commit-hash="${headCommitHash}"]`
      );

      if (commitNode) {
        // Add a tooltip showing the files in this commit
        const filesList = Object.keys(headCommit.files).join(", ");
        const existingTitle = commitNode.title;
        commitNode.title = `${existingTitle}\nFiles: ${filesList}`;

        // Make the current commit node slightly larger to emphasize it contains files
        commitNode.style.transform = "scale(1.1)";
      }

      // Display files in the local repository area
      Object.keys(headCommit.files).forEach((fileName) => {
        const fileEl = document.createElement("div");
        fileEl.classList.add("file", "committed");
        fileEl.textContent = fileName;
        fileEl.title = `File: ${fileName}\nCommit: ${headCommitHash}\nMessage: ${headCommit.message}`;
        localRepoFiles.appendChild(fileEl);
      });
    }

    // --- Remote Repository Files ---
    // Find the latest commits in all remote repository branches
    const remoteBranches = Object.keys(remoteRepo.branches);

    if (remoteBranches.length > 0) {
      // Get the current branch name from local repo to find matching remote branch
      const currentLocalBranch =
        localRepo.head && localRepo.head.type === "branch"
          ? localRepo.head.target
          : "main";

      // First try to use the matching branch, fallback to main or any available branch
      let remoteBranchToShow = remoteBranches.includes(currentLocalBranch)
        ? currentLocalBranch
        : remoteBranches.includes("main")
        ? "main"
        : remoteBranches[0];

      let remoteCommitHash = remoteRepo.branches[remoteBranchToShow];

      if (remoteCommitHash && remoteRepo.commits[remoteCommitHash]) {
        const remoteCommit = remoteRepo.commits[remoteCommitHash];

        // Display branch name in the remote repository area
        const branchIndicator = document.createElement("div");
        branchIndicator.classList.add("branch-indicator");
        branchIndicator.textContent = `Branch: ${remoteBranchToShow}`;
        remoteRepoFiles.appendChild(branchIndicator);

        // Display files in the remote repository area
        Object.keys(remoteCommit.files).forEach((fileName) => {
          const fileEl = document.createElement("div");
          fileEl.classList.add("file", "committed");
          fileEl.textContent = fileName;
          fileEl.title = `File: ${fileName}\nBranch: ${remoteBranchToShow}\nCommit: ${remoteCommitHash}\nMessage: ${remoteCommit.message}`;
          remoteRepoFiles.appendChild(fileEl);
        });
      }
    }
  }
  ...
  function getCommitPosition(commitHash, repoData, positionsCache) {
    ...
    // Improved horizontal positioning for better visualization
    // For main branch, keep commits in a straight line
    if (commit.branch === "main" || !commit.branch) {
      const pos = {
        x: 50, // Fixed x position for main branch
        y: 30 + depth * VERTICAL_SPACING, // Vertical positioning based on depth
        depth: depth,
      };
      positionsCache[commitHash] = pos;
      return pos;
    }

    // For feature branches, offset to the right
    let branchIndex = 1; // Default branch index
    if (commit.branch) {
      // Try to get consistent branch index
      const branchNames = Object.keys(repoData.branches);
      branchIndex = branchNames.indexOf(commit.branch);
      if (branchIndex === -1) branchIndex = 1;
      else if (branchIndex === 0) branchIndex = 1; // Ensure main branch (usually index 0) doesn't overlap
    }

    const pos = {
      x: 50 + branchIndex * HORIZONTAL_SPACING * 1.5, // Offset feature branches to the right
      y: 30 + depth * VERTICAL_SPACING, // Vertical positioning based on depth
      depth: depth,
    };
  ...
  function renderCommitsUI(repoContentElement, repoData) {
    ...
    Object.keys(repoData.commits).forEach((hash) =>
      getCommitPosition(hash, repoData, positions)
    );
    ...
    Object.values(repoData.commits).forEach((commit) => {
      ...
      (commit.parents || []).forEach((parentId) => {
        ...
        if (parentPos) {
          ...
          const branchClass = parentCommit
            ? getBranchClass(parentCommit.branch)
            : "main-branch"; // Default or parent's branch
    ...
    Object.entries(repoData.branches).forEach(([branchName, commitHash]) => {
      ...
      if (commitEl) {
        ...
        label.style.left = `${
          parseInt(commitEl.style.left, 10) + COMMIT_NODE_SIZE + 5
        }px`; // Position right of node
  ...
  function getBranchClass(branchName) {
    ...
    if (branchName === "main" || branchName === "origin/main")
      return "main-branch";
    if (branchName === "feature" || branchName === "origin/feature")
  ...
  function updateHeadPointerUI() {
    ...
    if (localRepo.head.type === "branch") {
      ...
      targetElement = localRepoContent.querySelector(
        `.commit-node[data-commit-hash="${commitHash}"]`
      );
    ...
    if (targetElement) {
      ...
      localHeadPointer.style.left = `${parseInt(
        targetElement.style.left,
        10
      )}px`;
      localHeadPointer.style.top = `${
        parseInt(targetElement.style.top, 10) - COMMIT_NODE_SIZE - 5
      }px`; // Adjust spacing
    } else {
  ...
  function findLatestCommit(repoData, branchName = null) {
    const targetBranch =
      ...
      (localRepo.head && localRepo.head.type === "branch"
        ? localRepo.head.target
        : "main"); // Default to main if head is detached
  ...
  function findCommonAncestor(commitHash1, commitHash2, repoData) {
    if (
      !commitHash1 ||
      !commitHash2 ||
      !repoData.commits[commitHash1] ||
      !repoData.commits[commitHash2]
    )
      return null;
    ...
    while (q.length > 0) {
      ...
      if (repoData.commits[current]) {
        // Avoid processing non-existent commits
  ...
  function handleInit() {
    ...
    logToTerminal(
      "Initialized empty Git repository in /visualizer/.git/",
      "output",
      "git init"
    );
    updateExplanationText(
      "Git repository created! The 'main' branch exists but has no commits yet. Add some files."
    );
    renderFilesUI();
    renderCommitsUI(localRepoContent, localRepo);
    renderCommitsUI(remoteRepoContent, remoteRepo);
    renderRepositoryFiles(); // Initialize repository files display
    updateRemoteUrlUI();
    enableCommands(true);
  ...
  function handleStatus() {
    ...
    if (stagedFiles.length > 0) {
      ...
      stagedFiles.forEach((name) =>
        logToTerminal(
          `\t${
            files[name].originalStatus === "untracked"
              ? "new file:"
              : "modified:"
          } ${name}`,
          "output"
        )
      ); // Show if new or modified when staged
    }
    ...
    if (modifiedFiles.length > 0) {
      ...
      modifiedFiles.forEach((name) =>
        logToTerminal(`\tmodified:   ${name}`, "warning")
      );
    }
    ...
    if (
      ...
      const headCommitHash =
        headType === "branch" ? localRepo.branches[headTarget] : headTarget;
      if (!headCommitHash) {
        logToTerminal("\nNo commits yet");
      }
    ...
    updateExplanationText(
      "Status shows differences between Working Dir, Staging Area, and your last commit (HEAD)."
    );
  }
  ...
  function handleLog() {
    ...
    if (!currentHash) {
      ...
      updateExplanationText(
        "Showing commit history. No commits have been made on this branch yet."
      );
      return;
    }
    ...
    while (currentHash && count < MAX_LOG) {
      ...
      if (
        ...
        headMarker = ` (HEAD -> ${localRepo.head.target}${
          localRepo.head.type === "commit" ? " [detached]" : ""
        })`;
      }
      ...
      Object.entries(localRepo.branches).forEach(([bName, bHash]) => {
        if (bHash === currentHash && bName !== localRepo.head.target) {
          // Don't repeat HEAD target
          branchMarkers += `, ${bName}`;
        }
      });
      Object.entries(localRepo.branches).forEach(([bName, bHash]) => {
      ...
      logToTerminal(
        `commit ${commit.hash}${headMarker}${branchMarkers}`,
        "warning"
      ); // Often yellow
      ...
      currentHash =
        commit.parents && commit.parents.length > 0 ? commit.parents[0] : null;
      count++;
    }
    if (currentHash) {
    ...
    updateExplanationText(
      "Showing commit history for the current branch/commit (most recent first). HEAD shows your current position."
    );
  }
  ...
  function handleCreateFile(
    ...
    if (!gitInitialized) {
      logToTerminal(
        "Create failed: Initialize Git first (`git init`).",
        "error"
      );
      return;
    }
    if (files[name]) {
      logToTerminal(
        `File '${name}' already exists. Use 'Modify' button.`,
        "warning"
      );
      return;
    }
    ...
    logToTerminal(
      `Created '${name}' in Working Directory.`,
      "output",
      `echo "${content}" > ${name}`
    ); // Simulate command
    updateExplanationText(
      `Created '${name}'. It's 'untracked' by Git. Use 'git add' to stage it.`
    );
    renderFilesUI();
  }
  ...
  function handleModifyFile(name = "index.js") {
    if (!gitInitialized || !files[name]) {
      logToTerminal(
        `Modify failed: File '${name}' does not exist or Git not initialized.`,
        "error"
      );
      return;
    }
    if (files[name].area !== "working" && files[name].area !== "staging") {
      // Can't modify stashed/committed directly
      logToTerminal(
        `Modify failed: Cannot directly modify '${name}' in its current state (${files[name].area}). Checkout first if needed.`,
        "error"
      );
      return;
    }
    ...
    if (files[name].status === "tracked" || files[name].status === "staged") {
      // If it was tracked or staged, modifying makes it 'modified' in working dir
      files[name].status = "modified";
    ...
    logToTerminal(
      `Modified '${name}' in Working Directory.`,
      "output",
      `vim ${name}`
    ); // Simulate command
    updateExplanationText(
      `Modified '${name}'. Use 'git add' to stage the changes.`
    );
    renderFilesUI();
  }
  ...
  function handleAddGitignore() {
    ...
    if (!files[fileName]) {
      ...
      if (
        files[fileName].status === "tracked" ||
        files[fileName].status === "staged"
      ) {
    ...
    updateExplanationText(
      `Added 'config.prop' to .gitignore. Git will now ignore this file. Don't forget to 'git add .gitignore' and commit it!`
    );
  ...
  function handleAdd(fileName) {
    ...
    if (fileName === ".") {
      ...
      if (ignoredFiles.has(fileName)) {
        ...
        updateExplanationText(
          `'${fileName}' is ignored by .gitignore. Use 'git add -f ${fileName}' to force add (not simulated here).`
        );
        return;
      }
      if (
        files[fileName].area === "working" &&
        (files[fileName].status === "modified" ||
          files[fileName].status === "untracked")
      ) {
        ...
        logToTerminal(
          `No changes detected for '${fileName}' in working directory.`,
          "output",
          commandStr
        );
        return;
      ...
      updateExplanationText(
        `File '${fileName}' not found in your working directory.`
      );
      return;
    }
    ...
    filesToAdd.forEach((name) => {
      ...
      files[name].originalStatus =
        files[name].originalStatus || files[name].status; // Store original if not already stored
    ...
    updateExplanationText(
      `${filesToAdd.join(
        ", "
      )} moved to the Staging Area. Ready to be committed.`
    );
    renderFilesUI();
  }
  ...
  function handleCommit(stageAllFirst = false) {
    ...
    const commandStr = stageAllFirst
      ? 'git commit -am "..."'
      : 'git commit -m "..."';
    ...
    if (stagedFiles.length === 0) {
      ...
      if (stageAllFirst && !isWorkingTreeClean()) {
        // -am specific message if only untracked exist
        logToTerminal('(use "git add" to track untracked files)', "warning");
      }
      updateExplanationText(
        "No changes added to the Staging Area. Use 'git add' first."
      );
      return;
    }
    ...
    const parentHash = localRepo.head
      ? localRepo.head.type === "branch"
        ? localRepo.branches[localRepo.head.target]
        : localRepo.head.target
      : null;
    ...
    const commitMessage = stageAllFirst
      ? `Commit ${commitId} via -am`
      : `Commit ${commitId}: Staged changes`; // Simple message
    const newCommit = {
      hash: commitHash,
    ...
    // Update working directory to reflect the committed state
    // In real Git, files remain in working directory but are now tracked and clean
    stagedFiles.forEach((name) => {
      // Keep a copy in the working directory that matches the committed version
      files[name].area = "working";
      files[name].status = "tracked"; // Now tracked (clean state)
      delete files[name].originalStatus; // Reset original status marker
    });

    // For visualization purposes, we should show that these files are now in the repository
    // This is handled by the commit snapshot in localRepo.commits[commitHash].files

    logToTerminal(
      `[${localRepo.head.target} ${commitHash}] ${commitMessage}`,
      "output",
      commandStr
    );
    logToTerminal(`${stagedFiles.length} file(s) changed.`, "output"); // Simplified status
    updateExplanationText(
      `Created commit ${commitHash.substring(0, 7)} on branch ${
        currentBranch || "[detached]"
      }. Staging Area is now empty. Files are now tracked in the Local Repository and remain in your Working Directory in a clean state.`
    );

    renderFilesUI();
    renderCommitsUI(localRepoContent, localRepo); // Re-render commits
    renderRepositoryFiles(); // Update repository files display
  }
  ...
  function handleBranchFeature() {
    ...
    if (!currentCommitHash) {
      logToTerminal(
        `Cannot create branch '${branchName}': The current HEAD is unborn. Commit first.`,
        "error",
        commandStr
      );
      updateExplanationText(
        `Cannot create a branch until the first commit is made.`
      );
      return;
    }
    ...
    updateExplanationText(
      `Created new branch 'feature'. It points to the same commit as HEAD (${currentCommitHash.substring(
        0,
        7
  ...
  function handleCheckout(target) {
    // Target can be branch name or commit hash
    if (!gitInitialized) {
    ...
    if (localRepo.branches[target]) {
      ...
      if (!isWorkingTreeClean()) {
        ...
        updateExplanationText(
          `You have uncommitted changes. Commit or stash them before switching branches to avoid losing work.`
        );
        return;
      }
    ...
    // Check if target is a commit hash (simplified check)
      ...
      localRepo.commits[target] ||
      Object.values(localRepo.commits).find((c) => c.hash.startsWith(target))
    ) {
      const fullHash =
        Object.values(localRepo.commits).find((c) => c.hash.startsWith(target))
          ?.hash || target;
      ...
      if (!localRepo.commits[fullHash]) {
        logToTerminal(
          `fatal: reference is not a tree: ${target}`,
          "error",
          commandStr
        );
        return;
      ...
      if (!isWorkingTreeClean()) {
        ...
        updateExplanationText(
          `You have uncommitted changes. Commit or stash them before checking out a commit.`
        );
        return;
      }
      ...
      logToTerminal(
        `Note: switching to '${fullHash.substring(0, 7)}'.\n`,
        "output",
        commandStr
      );
      logToTerminal(
      ...
      logToTerminal(
        `HEAD is now at ${fullHash.substring(0, 7)} ${
          localRepo.commits[fullHash].message
        }`
      );
    } else {
      logToTerminal(
      ...
      updateExplanationText(
        `Could not find a branch or commit matching '${target}'.`
      );
      return;
    }
    ...
    updateExplanationText(
      isBranchSwitch
        ...
        : `Switched to commit ${targetCommitHash.substring(
            0,
            7
          )}. You're in a 'detached HEAD' state. Working Directory matches this commit. New commits won't belong to any branch unless you create one.`
    );
  ...
  function handleMergeFeature() {
    if (
      ...
      logToTerminal(
        "Merge failed: Not on a branch or Git not initialized.",
        "error",
        "git merge feature"
      );
      return;
    ...
    if (!localRepo.branches["feature"]) {
      logToTerminal(
        "Merge failed: Branch 'feature' not found.",
        "error",
        "git merge feature"
      );
      return;
    ...
    if (!isWorkingTreeClean()) {
      logToTerminal(
        "Merge failed: You have uncommitted changes. Commit or stash first.",
        "error",
        "git merge feature"
      );
      return;
    ...
    if (currentCommitHash === featureCommitHash) {
      ...
      updateExplanationText(
        `Branch '${currentBranchName}' already contains all changes from 'feature'.`
      );
      return;
    }

    // Find common ancestor
    const commonAncestorHash = findCommonAncestor(
      currentCommitHash,
      featureCommitHash,
      localRepo
    );
    ...
    if (commonAncestorHash === currentCommitHash) {
      logToTerminal(
        `Updating ${currentCommitHash.substring(
          0,
          7
        )}..${featureCommitHash.substring(0, 7)}`
      );
      ...
      updateExplanationText(
        `Fast-forward merge completed. '${currentBranchName}' now points to the same commit as 'feature'.`
      );
    }
    // --- Three-way merge ---
    else {
      ...
      for (const name in currentCommit?.files || {}) {
        if (
          !baseCommit?.files ||
          !baseCommit.files[name] ||
          baseCommit.files[name].content !== currentCommit.files[name].content
        ) {
      ...
      for (const name in featureCommit?.files || {}) {
        ...
        if (changedInFeature) {
          if (
            mergedFiles[name] &&
            changedInCurrent &&
            mergedFiles[name].content !== featureContent
          ) {
      ...
      if (conflictDetected) {
        ...
        Object.entries(mergedFiles).forEach(([name, fileData]) => {
          files[name] = {
            ...
            status:
              fileData.status === "conflicted" ? "conflicted" : "modified", // Mark others as modified relative to old HEAD
            area: "working",
          };
        });
        logToTerminal(
        ...
        updateExplanationText(
          `Merge conflict! Files marked red have conflicts. Manually edit them (not simulated), 'git add' the resolved files, and then 'git commit'.`
        );
        ...
        updateExplanationText(
          `Successfully merged 'feature' into '${currentBranchName}' by creating a merge commit (${mergeCommitHash.substring(
            0,
            7
          )}).`
        );
  ...
  function handleRemoteAdd() {
    ...
    if (
      Object.keys(remoteRepo.commits).length === 0 &&
      localRepo.branches["main"] &&
      localRepo.commits[localRepo.branches["main"]]
    ) {
    ...
    logToTerminal(
      `Added remote 'origin'.`,
      "output",
      `git remote add origin ${localRepo.remoteUrl}`
    );
    updateExplanationText(
      `Connected local repository to a simulated remote named 'origin'. Now you can push/pull.`
    );
    updateRemoteUrlUI();
  ...
  function handlePush(setUpstream = false, branchToPush = null) {
    if (!gitInitialized || !localRepo.remoteUrl) {
      logToTerminal(
        "Push failed: No remote configured or Git not initialized.",
        "error",
        "git push"
      );
      return;
    ...
    const currentLocalBranch =
      branchToPush ||
      (localRepo.head.type === "branch" ? localRepo.head.target : null);
    if (!currentLocalBranch) {
      logToTerminal(
        "Push failed: You are in a detached HEAD state. Push requires a branch.",
        "error",
        "git push"
      );
      updateExplanationText(
        "Cannot push while in detached HEAD state. Checkout a branch first."
      );
      return;
    }
    const commandStr = setUpstream
    ...
    if (!localCommitHash) {
      logToTerminal(
        `Push failed: Branch '${currentLocalBranch}' has no commits.`,
        "error",
        commandStr
      );
      return;
    ...
    while (ancestor) {
      if (!remoteCommitsSet.has(ancestor) && remoteCommitHash) {
        // If remote branch exists and we find a local commit not on remote, that's fine for push
    ...
    if (
      ...
      !isAncestor(remoteCommitHash, localCommitHash, localRepo)
    ) {
      localNeedsPull = true;
    }
    ...
    if (localNeedsPull) {
      logToTerminal(
        `! [rejected]        ${currentLocalBranch} -> ${currentLocalBranch} (fetch first)`,
        "error",
        commandStr
      );
      logToTerminal(
        `error: failed to push some refs to '${localRepo.remoteUrl}'`
      );
      logToTerminal(
      ...
      updateExplanationText(
        `Push rejected! The remote branch has changes you don't have locally. Use 'git fetch' or 'git pull' first.`
      );
      return;
    }
    ...
    while (q.length > 0) {
      const current = q.shift();
      if (!current || visited.has(current) || remoteRepo.commits[current])
        continue; // Already exists remotely or visited

      visited.add(current);
      const commitData = localRepo.commits[current];
      if (commitData) {
        // Create a deep copy of the commit data, including files
        const commitCopy = {
          ...commitData,
          files: {},
        };

        // Deep copy all files in the commit
        if (commitData.files) {
          Object.entries(commitData.files).forEach(([fileName, fileData]) => {
            commitCopy.files[fileName] = { ...fileData };
          });
        }

        commitsToPush[current] = commitCopy;
    ...
    logToTerminal(
      ` * ${newBranchMarker}      ${currentLocalBranch} -> ${currentLocalBranch}`
    );
    if (setUpstream) {
      logToTerminal(
        `Branch '${currentLocalBranch}' set up to track remote branch '${currentLocalBranch}' from 'origin'.`
      );
    }

    updateExplanationText(
      `Pushed commits from local '${currentLocalBranch}' to remote 'origin/${currentLocalBranch}'.`
    );
  ...
  function handleFetch() {
    if (!gitInitialized || !localRepo.remoteUrl) {
      logToTerminal(
        "Fetch failed: No remote configured or Git not initialized.",
        "error",
        "git fetch"
      );
      return;
    ...
    if (!fetchedNewCommits && !fetchedNewBranches) {
      ...
      Object.entries(remoteRepo.branches).forEach(
        ([branchName, commitHash]) => {
          // Show summary of fetched branches - simplified
          logToTerminal(
            ` * [new branch]      ${branchName}     -> origin/${branchName}`
          ); // Assume new for simplicity
        }
      );
    }

    updateExplanationText(
      `Fetched changes from the remote 'origin'. New commits/branches are downloaded to your local repo (visible as 'origin/...'). Your working directory and local branches ('main', 'feature') are NOT updated yet. Use 'git merge origin/...' or 'git pull' to integrate.`
    );
  ...
  function handlePull() {
    if (!gitInitialized || !localRepo.remoteUrl) {
      logToTerminal(
        "Pull failed: No remote configured or Git not initialized.",
        "error",
        "git pull"
      );
      return;
    ...
    if (!localRepo.head || localRepo.head.type !== "branch") {
      logToTerminal(
        "Pull failed: You are not on a branch.",
        "error",
        "git pull"
      );
      return;
    ...
    if (!isWorkingTreeClean()) {
      logToTerminal(
        "Pull failed: You have uncommitted changes. Commit or stash first.",
        "error",
        "git pull"
      );
      return;
    ...
    if (!remoteCommit) {
      ...
      updateExplanationText(
        `Could not pull. Remote branch 'origin/${currentBranch}' doesn't seem to exist or tracking isn't set up.`
      );
      return;
    }
    ...
    if (localCommit === remoteCommit) {
      ...
      updateExplanationText(
        `Your local branch '${currentBranch}' is already synchronized with the remote 'origin/${currentBranch}'.`
      );
      return;
    }
    ...
    if (commonAncestorHash === localCommit) {
      logToTerminal(
        `Updating ${localCommit.substring(0, 7)}..${remoteCommit.substring(
          0,
          7
        )}`
      );
      ...
      updateExplanationText(
        `Pulled and fast-forwarded '${currentBranch}' to match 'origin/${currentBranch}'.`
      );
    }
    // Merge scenario (potentially with conflicts)
      ...
      logToTerminal(
        `Attempting merge of 'origin/${currentBranch}' into '${currentBranch}'...`
      );
      ...
      const mergedFilesSnapshot = {
        ...(localRepo.commits[localCommit]?.files || {}),
        ...(localRepo.commits[remoteCommit]?.files || {}),
      }; // Oversimplified merge
      ...
      logToTerminal(`Merge made by the 'recursive' strategy.`, "output"); // Simulate Git message
      updateExplanationText(
        `Pulled and merged remote changes into '${currentBranch}' creating merge commit ${mergeCommitHash.substring(
          0,
          7
        )}.`
      );
  ...
  function handleRemoteCommit() {
    ...
    if (!remoteMainHash) {
      logToTerminal(
        'Simulation failed: Remote "main" branch has no commits yet.',
        "warning"
      );
      return;
    }
    ...
    if (newCommit.files[fileNameToChange]) {
      ...
      newCommit.files["remote_file.txt"] = {
        content: "Added remotely",
        status: "tracked",
        area: "committed",
      };
    }
    ...
    logToTerminal(
      `Simulated: Another developer pushed commit ${newRemoteCommitHash.substring(
        0,
        7
    ...
    updateExplanationText(
      "Simulated a commit pushed by someone else to the remote 'main' branch. Use 'git fetch' or 'git pull' to get these changes."
    );
    renderCommitsUI(remoteRepoContent, remoteRepo); // Update remote view
    renderRepositoryFiles(); // Update repository files display
  }
  ...
  function handleStash() {
    ...
    if (
      Object.keys(workingChanges).length === 0 &&
      Object.keys(stagedChanges).length === 0
    ) {
      logToTerminal("No local changes to save", "output", "git stash");
      updateExplanationText(
        "No changes in the working directory or staging area to stash."
      );
      return;
    }
    ...
    logToTerminal(
      `Saved working directory and index state WIP on ${
        ...
        localRepo.commits[localRepo.head.target]?.hash.substring(0, 7) ||
        "Unborn"
      } ...`,
      "output",
      "git stash"
    ...
    updateExplanationText(
      `Stashed changes. Working directory and staging area are now clean (matching HEAD). Use 'git stash pop' to reapply.`
    );
    renderFilesUI();
  ...
  function handleStashPop() {
    ...
    logToTerminal(
      "Applying stashed changes... (Simplified simulation)",
      "output",
      "git stash pop"
    );
    ...
    for (const name in stashToPop.files) {
      ...
      if (
        files[name] &&
        files[name].content !== stashedData.content &&
        files[name].status !== "untracked" &&
        files[name].area === "working"
      ) {
    ...
    if (conflictSimulated) {
      ...
      updateExplanationText(
        "Stash applied, but conflicts occurred! Resolve conflicts in red files, then add and commit."
      );
    } else {
      logToTerminal("Changes applied from stash.");
      updateExplanationText(
        "Successfully applied stashed changes. Stash list is now empty."
      );
    }
  }
  ...
  function updateRemoteUrlUI() {
    remoteUrlDisplay.textContent = localRepo.remoteUrl
      ? `Connected: ${localRepo.remoteUrl}`
      : "Not Connected";

    // Update repository files display when remote connection changes
    renderRepositoryFiles();
  }
  ...
  btnCreateFile.onclick = () =>
    handleCreateFile("index.js", `console.log('Initial content');`);
  btnModifyFile.onclick = () => handleModifyFile("index.js");
  btnCreateConfig.onclick = () =>
    handleCreateFile("config.prop", "API_KEY=SECRET123");
  ...
  btnCheckoutCommit.onclick = () => {
    ...
    if (parentHash) {
      ...
      logToTerminal(
        "Cannot checkout parent: No parent commit found.",
        "warning"
      );
    }
  };
  ...
  btnClone.onclick = () => {
    alert("Clone simulation resets current state and copies remote. OK?");
    handleInit();
    handleRemoteAdd();
    handleFetch();
    handleCheckout("main");
